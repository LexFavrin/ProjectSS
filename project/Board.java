...
//    	
    ...
    public boolean isField(int row, int col, int height) {
    	if (isValidIndex(index(row, col, height))){
    		return true;
    	}
        return false;
    }
     ...
     * @return the mark on the field
     */
    //@ requires this.isField(row,col,height);
    //@ ensures \result == Mark.EMPTY || \result == Mark.XX || \result == Mark.OO;
    /*@pure*/
    public Mark getField(int row, int col, int height) {
    	assert isField(index(row, col, height));
    	System.out.println(index(row,col,height));
    	return fields[index(row,col,height)]; 
    }
    ...
    public boolean isEmptyField(int row, int col, int height) {
    	if (isField(row, col, height)){
    		return (this.getField(index(row, col, height)) == Mark.EMPTY);
    	}
    	return false;
    }
...
//     */
    public String toString() {
        ...
        for(int i = 0; i < DIM; i++){
            ...
            for(int z = 0; z < DIM; z++){
            	for(int j = 0; j < DIM; j++){
            		//System.out.println(getField(i,j,z));
